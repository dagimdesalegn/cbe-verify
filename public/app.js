...
function field(label, value) {
  return '<div><div class="text-xs text-gray-500 uppercase tracking-wide">' + escapeHtml(label) + '</div><div class="text-gray-100 mt-0.5 font-mono">' + escapeHtml(value == null ? '-' : value) + '</div></div>';
}
...
function renderHistory() {
  ...
  $('history').innerHTML = h.map((r) => '<div class="card rounded-xl px-4 py-3 flex items-center justify-between text-sm"><div class="flex items-center gap-3"><div class="w-2 h-2 rounded-full ' + (r.status === 'success' ? 'bg-green-500' : 'bg-yellow-500') + '"></div><div><div class="text-gray-200">' + escapeHtml(r.sender || '-') + ' to ' + escapeHtml(r.receiver || '-') + '</div><div class="text-xs text-gray-500 font-mono">' + escapeHtml(r.ref || '') + '</div></div></div><div class="text-right"><div class="text-gray-100">' + (r.amount ? Number(r.amount).toLocaleString() + ' ' + (r.currency || 'ETB') : '') + '</div><div class="text-xs text-gray-500">' + new Date(r.ts).toLocaleTimeString() + '</div></div></div>').join('');
}
