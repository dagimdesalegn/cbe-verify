...
const verifySchema = z
  .object({
    ...
    webhookUrl: z
      .string()
      .optional()
      .refine((v) => !v || isUrl(v), 'webhookUrl must be a valid http(s) URL'),
  })
  .refine((v) => v.referenceNumber || v.reference, {
    message: 'referenceNumber or reference is required',
  });

router.post(
  '/verify',
  requireApiKey('verification:write'),
  verifyLimiter,
  async (req, res, next) => {
    try {
...
router.get('/verify/history', requireApiKey('verification:read'), (_req, res) => {
  const rows = db
    .prepare(`SELECT * FROM verification_requests ORDER BY created_at DESC LIMIT 50`)
    .all() as any[];
  res.json({
    success: true,
...
function envelopeSuccess(requestId: string, result: any) {
  ...
  if (result.geoBlocked) item.geoBlocked = true;
...
function envelopeQueued(requestId: string, bank: string) {
  return {
    ...
    data: [],
    requestId,
    statusUrl: `/api/verify/${requestId}`,
