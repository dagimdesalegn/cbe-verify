...
function envelopeSuccess(requestId: string, result: any) {
  return {
    ...
    data: [{
      bank: result.bank, status: result.status, verified: result.verified,
      amount: result.amount, currency: result.currency,
      senderName: result.senderName, receiverName: result.receiverName,
      receiverAccount: result.receiverAccount, referenceNumber: result.referenceNumber,
